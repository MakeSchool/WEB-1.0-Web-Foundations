...
<Explanation>
On a US-style english keyboard layout, the angle brackets are on the comma (`,`) and period (`.`) keys, towards the bottom right side of the keyboard. Hold shift and press `,` for the `<` character. Shift + `.` for the `>` character.
</Explanation>
</TextResponse>
