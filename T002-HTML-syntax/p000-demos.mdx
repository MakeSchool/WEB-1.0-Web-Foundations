---
title: 'Demo Components'
slug: 'demos'
contentType: 'TextContent'
---

import HtmlDemo from '../HtmlDemo';
import MagicDemo, {MagicInput} from '../MagicDemo';
import JsDemo from '../JsDemo';

<MagicDemo
  initialState={{title: 'Intro to Recycling', tagline: 'Learn about recycling'}}
  input={(state, updateState) => (
    <>
      <MagicInput name="title" state={state} updateState={updateState} />
      <MagicInput name="tagline" state={state} updateState={updateState} />
    </>
  )}
  template={state => `<div>
  <h1>${state.title}</h1>
  <h2>${state.tagline}</h2>
</div>`}
/>

<MagicDemo
  initialState={{backgroundColor: '#EEEEEE', color: '#111111'}}
  input={(state, updateState) => (
    <>
      <MagicInput type="color" name="background color" state={state} updateState={updateState} />
      <MagicInput type="color" name="text color" state={state} updateState={updateState} />
    </>
  )}
  template={state =>
    `<style>
  div {
    background-color: ${state.backgroundColor};
  }
  h1, h2 {
    color: ${state.color};
  }
</style>
<div>
  <h1>Small Changes, Big Impacts</h1>
  <h2>An Exploration of Climate Action</h2>
</div>`
  }
/>

<JsDemo defer>

```js
alert('Hi there!');
```

</JsDemo>

<JsDemo>

```js
console.log('5 * 5 is ...');
console.log(5 * 5);
```

</JsDemo>

<JsDemo defer>

```html
<button>Click, and the monster will eat a list item</button>
<ul>
  <li>A crunchy carrot</li>
  <li>A tasty tangerine</li>
  <li>A sweet strawberry</li>
  <li>A perfect peach</li>
</ul>
```

```js
let button = document.querySelector('button');
let indexToEat = 0;
button.addEventListener('click', function () {
  let items = document.querySelectorAll('li');
  let snack = items[indexToEat];
  snack.innerText = 'The monster ate it!';
  indexToEat = indexToEat + 1;
});
```

</JsDemo>

<HtmlDemo>

```html
<ol>
  <li>Preheat oven to 350˚F (180˚C).</li>
  <li>
    In a bowl, add the bananas and mash until smooth. Add in the melted butter and stir until well
    combined.
  </li>
</ol>
```

</HtmlDemo>
